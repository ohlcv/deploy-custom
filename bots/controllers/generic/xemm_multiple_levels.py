...
class XEMMMultipleLevelsConfig(ControllerConfigBase):
    ...
    maker_connector: str = Field(
        ...
        client_data=ClientFieldData(
            prompt=lambda e: "Enter the maker connector: ", prompt_on_new=True
        ),
    )
    maker_trading_pair: str = Field(
        default="LBR-USDT",
        client_data=ClientFieldData(
            prompt=lambda e: "Enter the maker trading pair: ", prompt_on_new=True
        ),
    )
    taker_connector: str = Field(
        default="okx",
        client_data=ClientFieldData(
            prompt=lambda e: "Enter the taker connector: ", prompt_on_new=True
        ),
    )
    taker_trading_pair: str = Field(
        default="LBR-USDT",
        client_data=ClientFieldData(
            prompt=lambda e: "Enter the taker trading pair: ", prompt_on_new=True
        ),
    )
    ...
    max_profitability: Decimal = Field(
        default=0.01,
        client_data=ClientFieldData(
            prompt=lambda e: "Enter the maximum profitability: ", prompt_on_new=True
        ),
    )
    ...
    @validator(
        "buy_levels_targets_amount", "sell_levels_targets_amount", pre=True, always=True
    )
...
class XEMMMultipleLevels(ControllerBase):
    ...
    def determine_executor_actions(self) -> List[ExecutorAction]:
        ...
        stopped_buy_executors = self.filter_executors(
            executors=self.executors_info,
            filter_func=lambda e: e.is_done
            and e.config.maker_side == TradeType.BUY
        ...
        for target_profitability, amount in self.buy_levels_targets_amount:
            ...
            if (
                len(active_buy_executors_target) == 0
                and imbalance < self.config.max_executors_imbalance
            ):
                ...
                executor_actions.append(
                    CreateExecutorAction(
                        executor_config=config, controller_id=self.config.id
                    )
                )
        for target_profitability, amount in self.sell_levels_targets_amount:
            ...
            if (
                len(active_sell_executors_target) == 0
                and imbalance > -self.config.max_executors_imbalance
            ):
                config = XEMMExecutorConfig(
                    controller_id=self.config.id,
    ...
    def to_format_status(self) -> List[str]:
        all_executors_custom_info = pd.DataFrame(
            e.custom_info for e in self.executors_info
        )
        return [
