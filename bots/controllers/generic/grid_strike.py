...
class GridStrikeConfig(ControllerConfigBase):
    ...
    grid_ranges: List[GridRange] = Field(
        default=[
            GridRange(
                id="R0",
    ...
    time_limit: Optional[int] = Field(
        default=60 * 60 * 24 * 2, client_data=ClientFieldData(is_updatable=True)
    )
    activation_bounds: Decimal = Field(
        default=Decimal("0.01"), client_data=ClientFieldData(is_updatable=True)
    )
    min_spread_between_orders: Optional[Decimal] = Field(
        default=None, client_data=ClientFieldData(is_updatable=True)
    )
    min_order_amount: Optional[Decimal] = Field(
        default=Decimal("1"), client_data=ClientFieldData(is_updatable=True)
    )
    max_open_orders: int = Field(
        default=5, client_data=ClientFieldData(is_updatable=True)
    )
    grid_range_update_interval: int = Field(
        default=60, client_data=ClientFieldData(is_updatable=True)
    )
...
class GridStrike(ControllerBase):
    ...
    def _calculate_grid_config(self):
        ...
        if self.config.min_spread_between_orders:
            spread_between_orders = (
                self.config.min_spread_between_orders * self.get_mid_price()
            )
            step_proposed = max(
                self.trading_rules.min_price_increment, spread_between_orders
            )
        else:
            step_proposed = self.trading_rules.min_price_increment
        amount_proposed = (
            max(self.trading_rules.min_notional_size, self.config.min_order_amount)
            if self.config.min_order_amount
            else self.trading_rules.min_order_size
        )
        for grid_range in self.config.grid_ranges:
            if grid_range.active:
                total_amount = (
                    grid_range.total_amount_pct * self.config.total_amount_quote
                )
                theoretical_orders_by_step = (
                    grid_range.end_price - grid_range.start_price
                ) / step_proposed
                theoretical_orders_by_amount = total_amount / amount_proposed
                orders = int(
                    min(theoretical_orders_by_step, theoretical_orders_by_amount)
                )
                prices = Distributions.linear(
                    orders, float(grid_range.start_price), float(grid_range.end_price)
                )
                step = (
                    (grid_range.end_price - grid_range.start_price)
                    / grid_range.end_price
                    / orders
                )
                if orders == 0:
                    self.logger().warning(
                        f"Grid range {grid_range.id} has no orders, change the parameters "
    ...
    def get_balance_requirements(self) -> List[TokenAmount]:
        ...
        return [
            TokenAmount(
                base_currency, self.config.extra_balance_base_usd / self.get_mid_price()
            )
        ]
    ...
    async def update_processed_data(self):
        ...
        self.processed_data.update(
            {
                "mid_price": mid_price,
                "active_executors_order_placed": self.active_executors(
                    is_trading=False
                ),
                "active_executors_order_trading": self.active_executors(
                    is_trading=True
                ),
                "long_activation_bounds": mid_price
                * (1 - self.config.activation_bounds),
                "short_activation_bounds": mid_price
                * (1 + self.config.activation_bounds),
            }
        )
    ...
    def determine_create_executor_actions(self) -> List[ExecutorAction]:
        ...
        for level in self.grid_levels:
            if (
                level.side == TradeType.BUY and level.price >= long_activation_bounds
            ) or (
                level.side == TradeType.SELL and level.price <= short_activation_bounds
            ):
                levels_allowed.append(level)
        active_executors = (
            self.processed_data["active_executors_order_placed"]
            + self.processed_data["active_executors_order_trading"]
        )
        active_executors_level_id = [
            executor.custom_info["level_id"] for executor in active_executors
        ]
        levels_allowed = sorted(
            [
                level
                for level in levels_allowed
                if level.id not in active_executors_level_id
            ],
        ...
        for level in levels_allowed:
            if level.side == TradeType.BUY and level.price > mid_price:
                ...
                take_profit = max(
                    level.step * 2, ((level.price - mid_price) / mid_price) + level.step
                )
                trailing_stop = None
                ...
                entry_price = mid_price
                take_profit = max(
                    level.step * 2, ((mid_price - level.price) / mid_price) + level.step
                )
                # trailing_stop_ap = max(level.step * 2, ((mid_price - level.price) / mid_price) + level.step)
                # trailing_stop = TrailingStop(activation_price=trailing_stop_ap, trailing_delta=level.step / 2)
                trailing_stop = None
    ...
    def determine_stop_executor_actions(self) -> List[ExecutorAction]:
        long_activation_bounds = self.processed_data["long_activation_bounds"]
        short_activation_bounds = self.processed_data["short_activation_bounds"]
        active_executors_order_placed = self.processed_data[
            "active_executors_order_placed"
        ]
        non_active_ranges = [
            grid_range.id
            for grid_range in self.config.grid_ranges
        ...
        active_executor_of_non_active_ranges = [
            ...
            and executor.custom_info["level_id"].split("_")[0] in non_active_ranges
        ]
        long_executors_to_stop = [
            executor.id
            for executor in active_executors_order_placed
            if executor.side == TradeType.BUY
            and executor.config.entry_price <= long_activation_bounds
        ]
        short_executors_to_stop = [
            executor.id
            for executor in active_executors_order_placed
            if executor.side == TradeType.SELL
            and executor.config.entry_price >= short_activation_bounds
        ]
        ...
        return [
            StopExecutorAction(controller_id=self.config.id, executor_id=executor)
            for executor in list(executors_id_to_stop)
        ]
