...
def get_dca_distribution_inputs():
    with st.expander(t("DCA Builder"), expanded=True):
        ...
        with c1:
            st.header(t("DCA Distribution"))
            buy_order_levels = st.number_input(
                t("Number of Order Levels"), min_value=1, value=levels_def, help=t("Enter the number of order levels (e.g., 2).")
            )
            ...
            with col_spreads:
                (
                    ...
                    buy_spread_ratio,
                    buy_manual_spreads,
                ) = distribution_inputs(col_spreads, t("Spread"), buy_order_levels, dca_spreads)
            with col_amounts:
                (
                    ...
                    buy_amount_ratio,
                    buy_manual_amounts,
                ) = distribution_inputs(col_amounts, t("Amount"), buy_order_levels, dca_amounts)

        # Generate distributions
        spread_distributions = get_distribution(
            buy_spread_dist_type,
            buy_order_levels,
            buy_spread_start,
            buy_spread_base,
        ...
        amount_distributions = get_distribution(
            buy_amount_dist_type,
            buy_order_levels,
            buy_amount_start,
            buy_amount_base,
