...
def get_executors_distribution_inputs(use_custom_spread_units=False):
    ...
    with st.expander(t("Executors Configuration"), expanded=True):
        col_buy, col_sell = st.columns(2)
        with col_buy:
            st.header(t("Buy Order Settings"))
            buy_order_levels = st.number_input(
                t("Number of Buy Order Levels"),
                min_value=1,
                value=buy_order_levels_def,
                help=t("Enter the number of buy order levels (e.g., 2)."),
            )
        with col_sell:
            st.header(t("Sell Order Settings"))
            sell_order_levels = st.number_input(
                t("Number of Sell Order Levels"),
                min_value=1,
                value=sell_order_levels_def,
                help=t("Enter the number of sell order levels (e.g., 2)."),
            )
        ...
        with col_buy_spreads:
            (
                ...
                buy_spread_ratio,
                buy_manual_spreads,
            ) = distribution_inputs(col_buy_spreads, t("Spread"), buy_order_levels, buy_spreads)
        with col_buy_amounts:
            (
                buy_amount_dist_type,
                buy_amount_start,
                buy_amount_base,
                buy_amount_scaling,
                buy_amount_step,
                buy_amount_ratio,
                buy_manual_amounts,
            ) = distribution_inputs(col_buy_amounts, t("Amount"), buy_order_levels, buy_amounts_pct)
        with col_sell_spreads:
            (
                sell_spread_dist_type,
                sell_spread_start,
                sell_spread_base,
                sell_spread_scaling,
                sell_spread_step,
                sell_spread_ratio,
                sell_manual_spreads,
            ) = distribution_inputs(col_sell_spreads, t("Spread"), sell_order_levels, sell_spreads)
        with col_sell_amounts:
            (
                ...
                sell_amount_ratio,
                sell_manual_amounts,
            ) = distribution_inputs(col_sell_amounts, t("Amount"), sell_order_levels, sell_amounts_pct)

    # Generate distributions
    buy_spread_distributions = get_distribution(
        buy_spread_dist_type,
        buy_order_levels,
        buy_spread_start,
        buy_spread_base,
    ...
    sell_spread_distributions = get_distribution(
        sell_spread_dist_type,
        sell_order_levels,
        sell_spread_start,
        sell_spread_base,
    ...
    buy_amount_distributions = get_distribution(
        buy_amount_dist_type,
        buy_order_levels,
        buy_amount_start,
        buy_amount_base,
    ...
    sell_amount_distributions = get_distribution(
        sell_amount_dist_type,
        sell_order_levels,
        sell_amount_start,
        sell_amount_base,
