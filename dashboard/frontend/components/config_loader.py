...
from frontend.st_utils import get_backend_api_client, t
from frontend.utils.utils import generate_random_name
...
def get_default_config_loader(controller_name: str):
    ...
    with st.expander(t("configurations"), expanded=True):
        c1, c2 = st.columns(2)
        with c1:
            use_default_config = st.checkbox(t("use_default_config"), value=True)
        with c2:
            if not use_default_config:
                configs = [config for config in all_configs if config["controller_name"] == controller_name]
                if len(configs) > 0:
                    default_config = st.selectbox(t("select_config"), [config["id"] for config in configs])
