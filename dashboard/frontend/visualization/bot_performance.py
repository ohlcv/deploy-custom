...
def display_performance_summary_table(executors, executors_with_orders: pd.DataFrame):
    if not executors_with_orders.empty:
        ...
        grouped_executors = (
            executors.groupby(["controller_id", "controller_type", "exchange", "trading_pair"])
            .agg(
        ...
        grouped_executors["filled_amount_quote_sum"] = grouped_executors["filled_amount_quote"].apply(lambda x: f"$ {x:.2f}")
        grouped_executors["net_pnl_quote"] = grouped_executors["net_pnl_quote"].apply(lambda x: f"$ {x:.2f}")
        grouped_executors.rename(columns={"datetime": "start_datetime_utc", "id": "total_executors"}, inplace=True)
        ...
        st.dataframe(
            ...
            hide_index=True,
            column_config={
                "exchange": st.column_config.TextColumn(t("Exchange")),
                "trading_pair": st.column_config.TextColumn(t("Trading Pair")),
                "net_pnl_over_time": st.column_config.LineChartColumn(t("PnL Over Time"), y_min=y_min, y_max=y_max),
                "controller_id": st.column_config.TextColumn(t("Controller ID")),
                "controller_type": st.column_config.TextColumn(t("Controller Type")),
                "total_executors": st.column_config.NumberColumn(t("Total Positions")),
                "filled_amount_quote": st.column_config.NumberColumn(t("Total Volume"), format="$ %.2f"),
                "net_pnl_quote": st.column_config.NumberColumn(t("Net PnL"), format="$ %.2f"),
                "duration": st.column_config.TextColumn(t("Duration")),
            },
        )
...
def display_global_results(data_source: PerformanceDataSource):
    selected_controllers = st.multiselect(
        t("Select Controllers"),
        data_source.controllers_dict.keys(),
        help=t("Select one or more controllers from the table above to filter and analyze their specific performance."),
    )
    ...
    render_backtesting_metrics(summary_results=results_response, title=t("Global Metrics"))
    ...
    executors_df = data_source.get_executors_df(executors_filter=selected_controllers_filter, apply_executor_data_types=True)
...
def display_side_analysis(data_source: PerformanceDataSource, current_filter: Dict[str, Any] = None, is_long: bool = True):
    side_filter = current_filter.copy()
    side_filter["side"] = [TradeType.BUY] if is_long else [TradeType.SELL]
    executors_dict = data_source.get_executor_dict(side_filter, apply_executor_data_types=True, remove_special_fields=True)
    results = fetch_long_results(executors_dict) if is_long else fetch_short_results(executors_dict)
    if results:
        side_str = t("Long Positions") if is_long else t("Short Positions")
        st.write(f"### {side_str}")
        col1, col2, col3 = st.columns(3)
        col1.metric(label=t("Net PnL (Quote)"), value=f"{results['net_pnl_quote']:.2f}")
        col2.metric(label=t("Max Drawdown (USD)"), value=f"{results['max_drawdown_usd']:.2f}")
        col3.metric(label=t("Total Volume (Quote)"), value=f"{results['total_volume']:.2f}")
        col1.metric(label=t("Sharpe Ratio"), value=f"{results['sharpe_ratio']:.2f}")
        col2.metric(label=t("Profit Factor"), value=f"{results['profit_factor']:.2f}")
        col3.metric(label=t("Total Executors with Position"), value=results["total_executors_with_position"])
        fig = go.Figure(
            data=[go.Pie(labels=list(results["close_types"].keys()), values=list(results["close_types"].values()), hole=0.3)]
        )
        fig.update_layout(title=t("Close Types"))
        st.plotly_chart(fig, use_container_width=True)

        executors_df = data_source.get_executors_df(executors_filter=side_filter, apply_executor_data_types=True)
...
def display_execution_analysis(data_source: PerformanceDataSource):
    ...
    with col1:
        controller_id = st.selectbox(t("Controller ID"), data_source.controllers_dict.keys())
        config = data_source.controllers_dict[controller_id]
        connector_name = config["connector_name"]
        trading_pair = config["trading_pair"]
    with col2:
        interval = st.selectbox(t("Select interval"), list(intervals_to_secs.keys()), index=3)

    executors_filter = {
        "controller_id": [controller_id],
        "exchange": [connector_name],
        "trading_pair": [trading_pair],
        "close_type_name": ["TAKE_PROFIT", "TRAILING_STOP", "STOP_LOSS", "TIME_LIMIT", "EARLY_STOP", "FAILED"],
    }
    executors_df = data_source.get_executors_df(executors_filter=executors_filter, apply_executor_data_types=True)
    ...
    performance_tab, config_tab = st.tabs([t("Real Performance"), t("Controller Config")])
    with performance_tab:
        ...
        fig = create_backtesting_figure(df=candles_df, executors=executors_info_list, config=config)

        performance_section(performance_results, fig, t("Backtesting Metrics"))
        with config_tab:
            ...
            with col2:
                ...
                host = st.text_input(t("Host"), "localhost")
                if st.button(t("Upload to Backend API")):
                    backend_api_client = BackendAPIClient(host=host)
                    config["id"] = controller_id
                    backend_api_client.add_controller_config(config)
                    st.success(t("Config uploaded successfully!"))


@st.cache_data()
def fetch_market_data(params: Dict[str, Any] = None):
    with st.spinner(t("Loading market data from {connector}...").format(connector=params["connector"])):
        backend_api = get_backend_api_client()
...
def performance_section(results: dict, fig=None, title: str = t("Backtesting Metrics")):
    render_backtesting_metrics(results, title=t("Controller Performance"))
...
def display_executors_by_close_type_metrics(executors_df):
    ...
    for i, (key, value) in enumerate(executors_by_close_type.items()):
        with columns[i]:
            st.metric(label=t(key), value=f"${value:.2f}")


def display_tables_section(data_source: PerformanceDataSource):
    with st.expander(f"💵 {t('Trades')}"):
        trade_fill = data_source.load_trade_fill()
        if not trade_fill.empty:
            st.dataframe(
                trade_fill,
                use_container_width=True,
                column_config={col: t(col) for col in trade_fill.columns},
                hide_index=True,
            )
        else:
            st.info(t("No trade data available"))
        download_csv_button(trade_fill, "trade_fill", "download-trades")

    with st.expander(f"📩 {t('Orders')}"):
        orders = data_source.load_orders()
        if not orders.empty:
            st.dataframe(
                orders,
                use_container_width=True,
                column_config={col: t(col) for col in orders.columns},
                hide_index=True,
            )
        else:
            st.info(t("No order data available"))
        download_csv_button(orders, "orders", "download-orders")

    with st.expander(f"📈 {t('Executors')}"):
        executors = data_source.get_executors_df()
        if not executors.empty:
            st.dataframe(
                executors,
                use_container_width=True,
                column_config={col: t(col) for col in executors.columns},
                hide_index=True,
            )
        else:
            st.info(t("No executor data available"))
...
def format_duration(seconds):
    ...
    return f"{int(days)}{t('d')} {int(hours)}{t('h')} {int(minutes)}{t('m')}"
