...
def create_dca_graph(dca_inputs, dca_amount):
    ...
    for level in range(n_levels):
        ...
        break_even = (
            (sum([spread * amount for spread, amount in zip(dca_spreads_normalized, amounts)]) / sum(amounts)) - 0.01
            if sum(amounts) != 0
            else 0
        )
    ...
    fig.add_trace(
        go.Scatter(
            x=list(range(len(dca_spreads))),
            y=dca_spreads,
            name=t("Spread (%)"),
            mode="lines+markers",
            line=dict(width=3, color=tech_colors["spread"]),
        ),
        secondary_y=False,
    )
    fig.add_trace(
        go.Scatter(
            x=list(range(len(break_even_values))),
            y=break_even_values,
            name=t("Break Even (%)"),
            mode="lines+markers",
            line=dict(width=3, color=tech_colors["break_even"]),
        ),
        secondary_y=False,
    )
    fig.add_trace(
        go.Scatter(
            x=list(range(len(take_profit_values))),
            y=take_profit_values,
            name=t("Take Profit (%)"),
            mode="lines+markers",
            line=dict(width=3, color=tech_colors["take_profit"]),
        ),
        secondary_y=False,
    )
    ...
    fig.add_hline(
        y=last_break_even,
        line_dash="dash",
        annotation_text=f"{t('Global Break Even')}: {last_break_even:.2f} (%)",
        annotation_position="top left",
        line_color=tech_colors["break_even"],
    )
    fig.add_hline(
        y=stop_loss_value,
        line_dash="dash",
        annotation_text=f"{t('Stop Loss')}: {stop_loss_value:.2f} (%)",
        annotation_position="bottom right",
        line_color=tech_colors["stop_loss"],
    )
    ...
    fig.update_layout(
        title=t("Spread, Accumulated Amount, Break Even, and Take Profit by Order Level"),
        xaxis_title=t("Order Level"),
        yaxis_title=t("Spread (%)"),
        yaxis2_title=t("Amount (Quote)"),
        height=800,
        width=1800,
    ...
    max_secondary_value = max(
        max(accumulated_amount), max(dca_order_amounts), max(cum_unrealized_pnl)
    )  # Adjust based on your secondary y-axis data
    ...
    for i, loot in enumerate(loots_to_recover):
        fig.add_annotation(
            ...
            text=f"{circle_text}<br>{t('LTR')}: {round(loot, 2)}",  # Circle symbol and loot value in separate lines
            showarrow=False,
    ...
    dca_max_loss_annotation_text = f"{t('DCA Max Loss (Quote)')}: {dca_max_loss:.2f}"
    fig.add_annotation(
