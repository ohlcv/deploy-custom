...
def create_backtesting_figure(df, executors, config):
    ...
    fig = make_subplots(
        ...
        subplot_titles=(t("Candlestick"), t("PNL Quote")),
        row_heights=[0.7, 0.3],
    )
    ...
    layout_settings = get_default_layout(f"{t('Trading Pair')}: {config['trading_pair']}")
    ...
    fig.update_yaxes(title_text=t("PNL"), row=2, col=1)
    return fig
