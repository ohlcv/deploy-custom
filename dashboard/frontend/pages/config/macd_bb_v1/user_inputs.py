...
def user_inputs():
    ...
    with st.expander(t("MACD Bollinger Configuration"), expanded=True):
        c1, c2, c3, c4, c5, c6, c7 = st.columns(7)
        with c1:
            bb_length = st.number_input(t("Bollinger Bands Length"), min_value=5, max_value=1000, value=bb_length)
        with c2:
            bb_std = st.number_input(t("Standard Deviation Multiplier"), min_value=1.0, max_value=2.0, value=bb_std)
        with c3:
            bb_long_threshold = st.number_input(t("Long Threshold"), value=bb_long_threshold)
        with c4:
            bb_short_threshold = st.number_input(t("Short Threshold"), value=bb_short_threshold)
        with c5:
            macd_fast = st.number_input(t("MACD Fast"), min_value=1, value=macd_fast)
        with c6:
            macd_slow = st.number_input(t("MACD Slow"), min_value=1, value=macd_slow)
        with c7:
            macd_signal = st.number_input(t("MACD Signal"), min_value=1, value=macd_signal)
