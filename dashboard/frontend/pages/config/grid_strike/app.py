...
initialize_st_page(page_title=t("grid_strike_title"), icon="📊", initial_sidebar_state="expanded")
...
fig = make_subplots(
    rows=1,
    cols=1,
    subplot_titles=(f'{t("grid_strike_title")} - {inputs["trading_pair"]} ({inputs["interval"]})',),
)
...
def prepare_config_for_save(config):
    ...
    try:
        ...
        for grid_range in prepared_config["grid_ranges"]:
            ...
            # 确保所有数值都是浮点数而非Decimal
            grid_range["start_price"] = float(grid_range["start_price"])
            grid_range["end_price"] = float(grid_range["end_price"])
            grid_range["total_amount_pct"] = float(grid_range["total_amount_pct"])
        ...
        # 移除图表相关字段
        if "candles_connector" in prepared_config:
            del prepared_config["candles_connector"]
        if "interval" in prepared_config:
            del prepared_config["interval"]
        if "days_to_visualize" in prepared_config:
            del prepared_config["days_to_visualize"]

        return prepared_config
    except Exception as e:
        st.error(f"Error preparing config: {str(e)}")
        import traceback
...
render_save_config(st.session_state["default_config"]["id"], prepare_config_for_save(st.session_state["default_config"]))
