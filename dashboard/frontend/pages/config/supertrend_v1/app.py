...
initialize_st_page(page_title=t("SuperTrend V1"), icon="📈", initial_sidebar_state="expanded")
backend_api_client = get_backend_api_client()

st.text(t("This tool will let you create a config for SuperTrend V1 and visualize the strategy."))
get_default_config_loader("supertrend_v1")

inputs = user_inputs()
st.session_state["default_config"].update(inputs)

st.write(t("### Visualizing Supertrend Trading Signals"))
...
add_traces_to_fig(
    fig,
    get_supertrend_v1_signal_traces(candles, inputs["length"], inputs["multiplier"], inputs["percentage_threshold"]),
    row=1,
    col=1,
)
