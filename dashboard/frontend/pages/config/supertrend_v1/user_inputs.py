...
def user_inputs():
    ...
    with st.expander(t("SuperTrend Configuration"), expanded=True):
        c1, c2, c3 = st.columns(3)
        with c1:
            length = st.number_input(t("Supertrend Length"), min_value=1, max_value=200, value=length)
        with c2:
            multiplier = st.number_input(t("Supertrend Multiplier"), min_value=1.0, max_value=5.0, value=multiplier)
        with c3:
            percentage_threshold = st.number_input(t("Percentage Threshold (%)"), value=percentage_threshold) / 100
    return {
