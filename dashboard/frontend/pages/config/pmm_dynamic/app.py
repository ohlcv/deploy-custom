...
initialize_st_page(page_title="PMM Dynamic", icon="👩‍🏫")
backend_api_client = get_backend_api_client()

# Page content
st.text(t("This tool will let you create a config for PMM Dynamic, backtest and upload it to the Backend API."))
get_default_config_loader("pmm_dynamic")
# Get user inputs
inputs = user_inputs()
st.write(f"### {t('Visualizing MACD and NATR indicators for PMM Dynamic')}")
st.text(
    t(
        "The MACD is used to shift the mid price and the NATR to make the spreads dynamic. In the order distributions graph, we are going to see the values of the orders affected by the average NATR"
    )
)
...
with st.expander(t("Visualizing PMM Dynamic Indicators"), expanded=True):
    fig = make_subplots(
        rows=4,
        cols=1,
        shared_xaxes=True,
        vertical_spacing=0.02,
        subplot_titles=(t("Candlestick with Bollinger Bands"), "MACD", t("Price Multiplier"), t("Spreads Multiplier")),
    ...
    add_traces_to_fig(
        fig,
        get_macd_traces(
            df=candles, macd_fast=inputs["macd_fast"], macd_slow=inputs["macd_slow"], macd_signal=inputs["macd_signal"]
        ),
        row=2,
        col=1,
    )
    price_multiplier, spreads_multiplier = get_pmm_dynamic_multipliers(
        candles, inputs["macd_fast"], inputs["macd_slow"], inputs["macd_signal"], inputs["natr_length"]
    )
    add_traces_to_fig(
        fig, [go.Scatter(x=candles.index, y=price_multiplier, name=t("Price Multiplier"), line=dict(color="blue"))], row=3, col=1
    )
    add_traces_to_fig(
        fig, [go.Scatter(x=candles.index, y=spreads_multiplier, name=t("Base Spread"), line=dict(color="red"))], row=4, col=1
    )
...
st.write(f"### {t('Executors Distribution')}")
st.write(
    t(
        "The order distributions are affected by the average NATR. This means that if the first order has a spread of 1 and the NATR is 0.005, the first order will have a spread of 0.5% of the mid price."
    )
)
buy_spread_distributions, sell_spread_distributions, buy_order_amounts_pct, sell_order_amounts_pct = (
...
with st.expander(t("Executor Distribution:"), expanded=True):
    ...
    st.write(f"{t('Average NATR')}: {natr_avarage:.2%}")
    fig = create_executors_distribution_traces(
        buy_spreads, sell_spreads, inputs["buy_amounts_pct"], inputs["sell_amounts_pct"], inputs["total_amount_quote"]
    )
    st.plotly_chart(fig, use_container_width=True)
