...
def user_inputs():
    ...
    with st.expander(t("PMM Dynamic Configuration"), expanded=True):
        c1, c2, c3, c4 = st.columns(4)
        with c1:
            macd_fast = st.number_input(t("MACD Fast Period"), min_value=1, max_value=200, value=macd_fast)
        with c2:
            macd_slow = st.number_input(t("MACD Slow Period"), min_value=1, max_value=200, value=macd_slow)
        with c3:
            macd_signal = st.number_input(t("MACD Signal Period"), min_value=1, max_value=200, value=macd_signal)
        with c4:
            natr_length = st.number_input(t("NATR Length"), min_value=1, max_value=200, value=natr_length)
