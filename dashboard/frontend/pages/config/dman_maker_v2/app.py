...
initialize_st_page(page_title="D-Man Maker V2", icon="🧙‍♂️")
backend_api_client = get_backend_api_client()

# Page content
st.text(t("This tool will let you create a config for D-Man Maker V2 and upload it to the BackendAPI."))
...
st.write(f"### {t('Visualizing DCA Distribution for specific Executor Level')}")
st.write("---")
buy_order_levels = len(inputs["buy_spreads"])
sell_order_levels = len(inputs["sell_spreads"])

buy_executor_levels = [f"{t('BUY')}_{i}" for i in range(buy_order_levels)]
sell_executor_levels = [f"{t('SELL')}_{i}" for i in range(sell_order_levels)]
c1, c2 = st.columns(2)
with c1:
    executor_level = st.selectbox(t("Executor Level"), buy_executor_levels + sell_executor_levels)
...
with c2:
    st.metric(label=t("DCA Amount"), value=f"{dca_amount:.2f}")
