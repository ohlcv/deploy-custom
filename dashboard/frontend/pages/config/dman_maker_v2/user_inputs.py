...
def user_inputs():
    connector_name, trading_pair, leverage, total_amount_quote, position_mode, cooldown_time, executor_refresh_time, _, _, _ = (
    ...
    with st.expander(t("Custom D-Man Maker V2 Settings")):
        c1, c2 = st.columns(2)
        with c1:
            top_executor_refresh_time = st.number_input(t("Top Refresh Time (minutes)"), value=60) * 60
        with c2:
            executor_activation_bounds = st.number_input(t("Activation Bounds (%)"), value=0.1) / 100
    # Create the config
    config = {
