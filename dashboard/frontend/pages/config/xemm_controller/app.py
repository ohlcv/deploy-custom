...
initialize_st_page(page_title=t("xemm_controller_title"), icon="⚡️")

# Page content
st.text(t("This tool will let you create a config for XEMM Controller and upload it to the BackendAPI."))
st.write("---")
c1, c2, c3, c4, c5 = st.columns([1, 1, 1, 1, 1])

with c1:
    maker_connector = st.text_input(t("Maker Connector"), value="kucoin")
    maker_trading_pair = st.text_input(t("Maker Trading Pair"), value="LBR-USDT")
with c2:
    taker_connector = st.text_input(t("Taker Connector"), value="okx")
    taker_trading_pair = st.text_input(t("Taker Trading Pair"), value="LBR-USDT")
with c3:
    min_profitability = st.number_input(t("Min Profitability (%)"), value=0.2, step=0.01) / 100
    max_profitability = st.number_input(t("Max Profitability (%)"), value=1.0, step=0.01) / 100
with c4:
    buy_maker_levels = st.number_input(t("Buy Maker Levels"), value=1, step=1)
    buy_targets_amounts = []
    c41, c42 = st.columns([1, 1])
    for i in range(buy_maker_levels):
        with c41:
            target_profitability = st.number_input(f"{t('Target Profitability')} {i + 1} B% ", value=0.3, step=0.01)
        with c42:
            amount = st.number_input(f"{t('Amount')} {i + 1}B {t('Quote')}", value=10, step=1)
        buy_targets_amounts.append([target_profitability / 100, amount])
with c5:
    sell_maker_levels = st.number_input(t("Sell Maker Levels"), value=1, step=1)
    sell_targets_amounts = []
    c51, c52 = st.columns([1, 1])
    for i in range(sell_maker_levels):
        with c51:
            target_profitability = st.number_input(f"{t('Target Profitability')} {i + 1}S %", value=0.3, step=0.001)
        with c52:
            amount = st.number_input(f"{t('Amount')} {i + 1} S {t('Quote')}", value=10, step=1)
...
def create_order_graph(order_type, targets, min_profit, max_profit):
    ...
    fig.add_trace(
        go.Bar(x=x_labels, y=y_values, width=0.01, name=f"{order_type.capitalize()} Targets", marker=dict(color="gold"))
    )
    ...
    fig.add_shape(
        type="line",
        x0=min_profit_percent,
        y0=0,
        x1=min_profit_percent,
        y1=max(y_values, default=10),
        line=dict(color="red", width=2),
        name="Min Profitability",
    )
    fig.add_shape(
        type="line",
        x0=max_profit_percent,
        y0=0,
        x1=max_profit_percent,
        y1=max(y_values, default=10),
        line=dict(color="red", width=2),
    ...
    title_key = (
        "Buy Order Distribution with Profitability Targets"
        if order_type.lower() == "buy"
        else "Sell Order Distribution with Profitability Targets"
    )
    fig.update_layout(
        title=t(title_key),
        xaxis=dict(
        ...
        yaxis=dict(title=t("Order Amount")),
        height=400,
        width=600,
    )
...
with c1:
    config_base = st.text_input(
        t("Config Base"), value=f"xemm-{maker_connector}-{taker_connector}-{maker_trading_pair.split('-')[0]}"
    )
with c2:
    config_tag = st.text_input(t("Config Tag"), value="1.1")
...
with c3:
    upload_config_to_backend = st.button(t("Upload Config to BackendAPI"))
